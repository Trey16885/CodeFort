...
Every time Mistral updates, CodeFort is already updated — the `-latest` aliases mean the fort picks up each new release automatically, with no code change. The lineup is fixed: it belongs to the deployment, not to whoever opens the page.

## What CodeFort can do

- **Many tasks, not one project.** Each task is a named brief with its own workspace, its own files and its own discussion. Switch between them from the sidebar and the whole workspace swaps under you; they persist across reloads. Switching is locked while a run is in progress.
...
```bash
npm test        # 58 headless checks: VFS, shell, tasks, publish bundler, accounts
...
assets/js/
  ...
  tasks.js                 many tasks, each with its own workspace
